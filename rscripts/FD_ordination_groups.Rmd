...
# mean_traits_groups <- read.csv("./output/functional_measures_transects_cwm_groups.csv")
```
...
## env covariates territory
# sum(unique(covariates_terr$sp_name) %in% species_g2$sp2)
...
row3 <- ggplot() + annotate(geom = 'text', x=1, y=1, label="Dwellers", angle = 90, , size = 8) + theme_void() 
...
ggsave(filename = "./plots/PCoA_allplots_allgroups.png", 
        dpi = 900, height = 22, width = 16)
```


## plot only the groups and grid - point scale
...
## get row names
col1 <- ggplot() + annotate(geom = 'text', x=1, y=1, label="Grid Scale", angle = 90, size = 8) + theme_void() 
col3 <- ggplot() + annotate(geom = 'text', x=1, y=1, label="Point Scale", angle = 90, size = 8) + theme_void() 
row2 <- ggplot() + annotate(geom = 'text', x=1, y=1, label="Exploiters", size = 8) + theme_void() 
row3 <- ggplot() + annotate(geom = 'text', x=1, y=1, label="Dwellers", size = 8) + theme_void() 
row4 <- ggplot() + annotate(geom = 'text', x=1, y=1, label="Avoiders", size = 8) + theme_void()
```
...
plotlist <- list(a = col1, c = col3,
                 e = row2, f = row3, g = row4, 
                 l = plot_g1_grid, o = plot_g1_pts, 
                 p = plot_g2_grid, r = plot_g2_pts,
                 s = plot_g3_grid, v = plot_g3_pts)

wrap_plots(plotlist, guides = 'collect', design = layoutplot) +
  plot_layout(axis_titles = "collect", heights = c(1, 0.1 , 3, 0.1, 3, 0.1, 3, 0.1)) 
  # plot_annotation(tag_levels = 'A') 
  
ggsave(filename = "./plots/PCoA_2scales_groups.png", 
        dpi = 900, height = 10, width = 16)
```



## plot territory scale
...
plotlist <- list(i = plot_terr, m = plot_g1_terr, 
                 q = plot_g2_terr, t = plot_g3_terr)

wrap_plots(plotlist, guides = 'collect', design = layoutplot) +
  plot_layout(axis_titles = "collect", heights = c(1, 1, 1, 1)) 
  # plot_annotation(tag_levels = 'A') 
  
ggsave(filename = "./plots/PCoA_terrplots_allgroups.png", 
        dpi = 900, height = 12, width = 12)
```
