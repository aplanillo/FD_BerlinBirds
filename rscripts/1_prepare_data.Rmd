...
remove incomplete data and get a table with the centroids
```{r}
data_points <- birds_locations %>% 
  filter(transect != "be1") %>%  ## we remove transect 1 because it was incomplete
  filter(!sp_name %in% c("Apus apus", "Passer domesticus"))  # these species do not have reliable data
  
nrow(data_points) # total number of points after filtering
# [1] 13220
```

### extract environmental covariates 
```{r}
mean_urban_territories2 <- terra::extract(urban_variables_100m, data_points, fun = mean, na.rm = TRUE)
mean_veg_territories2 <- terra::extract(veg_variables_100m, data_points, fun = mean, na.rm = TRUE)

birds_terr_var2 <- cbind(data_points, mean_urban_territories2, mean_veg_territories2) %>% 
  dplyr::select(-ID, -ID.1)

summary(birds_terr_var2)

terr_variables2 <- birds_terr_var2 %>% 
  st_drop_geometry() %>% 
...
# write.csv(terr_variables2, "./data/territories_points_environmental_variables.csv", row.names = FALSE)
```
...
## explanatory variables at territory territory scales
terr_variables2 <- read.csv("./data/territories_points_environmental_variables.csv")
head(terr_variables2)
...
```{r cor territory point scale}
colnames(terr_variables2)

## get urbanization variables
varcor_terr2_urb <- terr_variables2 %>% ungroup() %>% 
  dplyr::select(imperv, pop_den, tcd, light, noise)  
  
## get vegetation structure variables
varcor_terr2_veg <- terr_variables2 %>% ungroup() %>% 
  dplyr::select(veg_pca1, veg_pca2, ndvi, glcm, glcm_hmg)  

cor_table_terrurb2 <- cor(varcor_terr2_urb, use = "pairwise.complete.obs")
cor_table_terrveg2 <- cor(varcor_terr2_veg, use = "pairwise.complete.obs")

## Plotting
## Urban
ggcorrplot(cor_table_terrurb2, hc.order = FALSE, type = "lower",
           lab = TRUE, outline.color = "white", 
           lab_size = 6, tl.cex = 18)

ggsave(filename = "./plots/Plot_correlation_urban_variables_territory_points.png",
       dpi = 600, height = 6, width = 6)

## Vegetation structure
ggcorrplot(cor_table_terrveg2, hc.order = FALSE, type = "lower",
           lab = TRUE, outline.color = "white", 
           lab_size = 6, tl.cex = 18)

ggsave(filename = "./plots/Plot_correlation_veg_variables_terrotory_points.png",
       dpi = 600, height = 6, width = 6)
```
