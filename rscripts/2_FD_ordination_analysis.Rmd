...
# 2.3 PCoA Territory scale points

```{r}
## env covariates territory points
covariates_terrpts <- read.csv("./data/territories_points_environmental_variables.csv") 
```

```{r}
covariates_terrpts_mean <- covariates_terrpts %>% 
  dplyr::select(-nvisits, -visit) %>% 
  group_by(transect) %>% 
  summarise(across(where(is.numeric), list(mean = ~mean(., na.rm = TRUE))))

## merge together to make sure the order is correct
all_data_terr2 <- bird_abund %>% 
  left_join(covariates_terrpts_mean, by = "transect") %>% 
  left_join(mean_traits_transect, by = "transect") %>% 
  column_to_rownames("transect")
...
## divide into the proper tables
species_matrix_terr2 <- all_data_terr2[,1:95]
env_matrix_terr2 <- all_data_terr2[,96:105]
traits_matrix_terr2 <-all_data_terr2[,106:110]

str(species_matrix_terr2)
str(env_matrix_terr2)
str(traits_matrix_terr2)
```

### Run ordination
```{r}
## species ordination
PCoA_communitycomposition <- capscale(species_matrix_terr2 ~ 1, dist="bray", metaMDS = TRUE)

## add environmetal variables
ef <- envfit(PCoA_communitycomposition, env_matrix_terr2, permu = 999, na.rm = T)
ef

## Add traits 
tq <- envfit(PCoA_communitycomposition, traits_matrix_terr2, permu = 999)

# Add species 
spf <- envfit(PCoA_communitycomposition, species_matrix_terr2, permu = 999)
spf
...
ggsave(plot = myplot, 
       filename = "./plots/PCoA_terr_points_scale.png", 
       dpi = 900, height = 8, width = 8)
```
